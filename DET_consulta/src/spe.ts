...
export interface SpePageProgress {
  pageNumber: number;
  pageLength: number;
...
export type SpePageFetcher = (pageNumber: number) => Promise<SpeSearchResponse>;
...
export async function fetchAllSpeProcurations(
  ...
  const records = await collectSpeProcurationRecords(
    (pageNumber) => fetchSpePage(page, pageNumber),
    async (progress) => {
      await logger.log(
        "info",
        `SPE pagina ${progress.pageNumber}: ${progress.pageLength} procuracoes (${progress.recordsCollected}/${progress.expectedTotal ?? "?"}).`,
      );
    },
  );
...
export async function collectSpeProcurationRecords(
  fetchPage: SpePageFetcher,
  onProgress?: (progress: SpePageProgress) => void | Promise<void>,
): Promise<SpeProcurationRecord[]> {
