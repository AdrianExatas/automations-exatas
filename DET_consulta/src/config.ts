...
export async function loadConfig(env: NodeJS.ProcessEnv = process.env): Promise<AppConfig> {
  ...
  return {
    ...
    browserMode: browserMode(env.BROWSER_MODE),
    dominioDsn: env.DOMINIO_ODBC_DSN?.trim() || "Contabil Oficial",
    dominioUser: env.DOMINIO_USER?.trim() || "EXTERNO",
    dominioPassword: env.DOMINIO_PASSWORD ?? "externo",
    filterDominioActive: booleanFlag(env.DOMINIO_FILTER_ACTIVE, true),
  };
}
...
function browserMode(value: string | undefined): AppConfig["browserMode"] {
  const normalized = value?.trim().toLowerCase() || "cdp";
  if (normalized !== "cdp" && normalized !== "playwright") {
    throw new Error("BROWSER_MODE deve ser 'cdp' ou 'playwright'.");
  }
  return normalized;
}
