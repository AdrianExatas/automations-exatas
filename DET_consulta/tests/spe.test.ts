...
import {
  collectSpeProcurationRecords,
  mapSpeRecordsToTargets,
  parseSpeSearchResponse,
} from "../src/spe.js";
...
describe("SPE", () => {
  ...
  test("pagina ate atingir totalItems", async () => {
    const calls: number[] = [];
    const records = await collectSpeProcurationRecords(async (pageNumber) => {
      calls.push(pageNumber);
      const count = pageNumber === 1 ? 100 : 50;
      return {
        content: Array.from({ length: count }, (_, index) => ({
          uid: `${pageNumber}-${index}`,
          status: 0,
        })),
        pageMetadata: {
          pageSize: 100,
