...
from .models import DownloadResult, NFSeItem
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            ...
            last_error = RuntimeError(
                f"Erro {stage}: HTTP {exc.code} em {shorten_url(url)} apos {elapsed:.1f}s. {details[:300]}"
            )
            break
            ...
            last_error = RuntimeError(
                f"Erro {stage}: {type(exc).__name__} em {shorten_url(url)} apos {elapsed:.1f}s. {reason}"
            )
            if attempt >= attempts:
                break
            backoff = backoff_for_retry(attempt - 1)
            print(
                f"Nova tentativa de {stage} ({attempt + 1}/{attempts}) em {backoff}s para {shorten_url(url)}"
            )
            time.sleep(backoff)
...
def dismiss_certificate_dialog(window: Any) -> bool:
    for button_title in ("Cancelar", "Fechar", "OK"):
        try:
...
def ensure_print_layout(page: "Page") -> None:
    ...
    if hidden_print_visible:
        raise RuntimeError("Layout de impressao invalido: elementos com 'hidden-print' continuam visiveis.")


def save_pdf(page: "Page", html: str, target_file: Path, *, timeout: int, retries: int) -> None:
    page.set_content(
    ...
    *,
    ...
    if needs_pdf:
        try:
            ...
            raise RuntimeError("playwright nao esta instalado para gerar PDF.") from exc
    else:
        sync_playwright = None

    if sync_playwright is None:
        for index, item in enumerate(items, start=1):
            print(f"[{index}/{len(items)}] Processando NFSe {item.numero} ({item.codigo_verificacao})...")
            try:
                if file_format in {"xml", "ambos"}:
                    xml_url = build_xml_url(digits_only(item.cnpj_emissor), item.codigo_verificacao, item.numero)
                    save_xml(
                        xml_url,
                        xml_dir / f"{file_stem(item.numero, item.codigo_verificacao)}.xml",
                        timeout=http_timeout,
                        retries=retries,
                    )
            except RuntimeError as exc:
                failures += 1
                print(f"[{index}/{len(items)}] Falha na NFSe {item.numero} ({item.codigo_verificacao}): {exc}")
                continue
            successes += 1
        return DownloadResult(successes=successes, failures=failures, output_dir=output_dir)
    ...
    with sync_playwright() as playwright:
        ...
        try:
            for index, item in enumerate(items, start=1):
                print(f"[{index}/{len(items)}] Processando NFSe {item.numero} ({item.codigo_verificacao})...")
                cnpj = digits_only(item.cnpj_emissor)
                codigo = item.codigo_verificacao
                numero = item.numero
                ...
                try:
                    if file_format in {"xml", "ambos"}:
                        ...
                        save_xml(
                            xml_url,
                            xml_dir / f"{file_stem(numero, codigo)}.xml",
                            timeout=http_timeout,
                            retries=retries,
                        )
                    ...
                    if needs_pdf and page is not None:
                        ...
                        pdf_path = pdf_dir / pdf_file_name(numero, codigo)
                        save_pdf(
                            page,
                            html,
                            pdf_path,
                        ...
                        if rename_callback is not None:
                            rename_callback(pdf_path)
                except RuntimeError as exc:
                    failures += 1
                    print(f"[{index}/{len(items)}] Falha na NFSe {numero} ({codigo}): {exc}")
                    continue
