...
export class EmpresaSelectionPage {
  ...
  private async openSelectionModalWithFallback(): Promise<void> {
    ...
    try {
      ...
      // String evita o helper __name injetado pelo tsx/esbuild no page.evaluate.
      await this.page.evaluate(`(() => {
        const opener = window.ExibeModalSelecionaParceiroEmpresaMenuLateral;
  ...
  async searchCnpj(cnpj: string): Promise<void> {
    ...
    await this.page.evaluate(
      `(value) => {
        const input = document.querySelector("#modalSelecionaParceiroEmpresaSelecao #txtBuscaConteudoMenuLateral");
        if (!input) return;
        input.value = value;
        input.dispatchEvent(new Event("input", { bubbles: true }));
        input.dispatchEvent(new Event("change", { bubbles: true }));
        input.dispatchEvent(new KeyboardEvent("keyup", { key: "Enter", bubbles: true }));
        const filter = window.FiltraEmpresaSelecaoMenuLateral;
