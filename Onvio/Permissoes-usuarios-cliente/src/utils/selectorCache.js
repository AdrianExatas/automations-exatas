...
const cacheConfig = config.cache || { enabled: true, ttl: 3600000, maxSize: 1000 };
...
function loadCache() {
  ...
  try {
    ...
    logger.warn('Erro ao carregar cache', { error: error.message, cacheFile });
  }
}
...
function saveCache() {
  ...
  try {
    ...
    logger.warn('Erro ao salvar cache', { error: error.message, cacheFile });
  }
}
 ...
 */
function generateCacheKey(field, selector) {
 ...
 */
function getCached(field, selector) {
 ...
 */
function setCached(field, selector, success, metadata = {}) {
  if (!cacheConfig.enabled) return;
 ...
 */
function getOrderedSelectors(field, selectors) {
  ...
  const withPriority = selectors.map(selector => {
    const cached = getCached(field, selector.selector || selector);
 ...
 */
function invalidateCache(field = null) {
  if (field) {
    const keysToDelete = [];
    memoryCache.forEach((value, key) => {
