 ...
 */
export function validateUserDataIntegrity(userData: UserData): {
  isValid: boolean;
  ...
  if (!userData || typeof userData !== 'object') {
    ...
    return { isValid: false, issues, checksum: '' };
  }
 ...
 */
export function validateMultipleUsersIntegrity(usersArray: UserData[]): {
  total: number;
  valid: number;
  invalid: number;
  issues: Array<{ index: number; user: string; issues?: ValidationError[]; type?: string; message?: string }>;
  checksums: Array<{ index: number; checksum: string }>;
} {
  const results = {
    total: usersArray.length,
    valid: 0,
    invalid: 0,
    issues: [] as Array<{ index: number; user: string; issues?: ValidationError[]; type?: string; message?: string }>,
    checksums: [] as Array<{ index: number; checksum: string }>
  };
  ...
  const checksumMap = new Map<string, number>();
  results.checksums.forEach(({ index, checksum }) => {
    if (checksumMap.has(checksum)) {
      results.issues.push({
        index,
        user: usersArray[index]?.nome || `Usuário ${index + 1}`,
        type: 'duplicate',
 ...
 */
export function detectInconsistencies(userData: UserData): Array<{
  type: string;
  field: string;
  message: string;
 ...
 */
export function generateIntegrityReport(usersArray: UserData[]): any {
  const validation = validateMultipleUsersIntegrity(usersArray);
  const allInconsistencies: Array<{ index: number; user: string; inconsistencies: Array<{ type: string; field: string; message: string; severity: string }> }> = [];
...
function generateRecommendations(
  validation: ReturnType<typeof validateMultipleUsersIntegrity>,
  inconsistencies: Array<{ index: number; user: string; inconsistencies: Array<{ type: string; field: string; message: string; severity: string }> }>
): Array<{ type: string; priority: string; message: string }> {
 ...
 */
export function validateBeforeSave(usersArray: UserData[]): {
  canSave: boolean;
  validation: ReturnType<typeof validateMultipleUsersIntegrity>;
  report: ReturnType<typeof generateIntegrityReport>;
} {
  const validation = validateMultipleUsersIntegrity(usersArray);
  ...
  if (report.inconsistencies.length > 0) {
    const highSeverity = report.inconsistencies.filter((item: { inconsistencies: Array<{ severity: string }> }) =>
      item.inconsistencies.some((inc: { severity: string }) => inc.severity === 'high')
    );
    
    if (highSeverity.length > 0) {
      logger.warn('Inconsistências de alta severidade detectadas', {
        count: highSeverity.length,
        inconsistencies: highSeverity.map((item: { user: string; inconsistencies: Array<{ severity: string }> }) => ({
          user: item.user,
          issues: item.inconsistencies.filter((inc: { severity: string }) => inc.severity === 'high')
        }))
      });
    }
  }
