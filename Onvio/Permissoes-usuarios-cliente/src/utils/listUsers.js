 ...
 * @returns {Array} Lista de objetos com informações dos usuários {id, nome, email, editButton}
 */
async function listAllUsers(page) {
  const users = [];
  ...
  try {
    ...
    try {
      ...
      logger.warn('Método 1 falhou', { error: e.message, method: 1 });
    }
    ...
    if (rows.length === 0) {
      try {
        ...
        logger.warn('Método 2 falhou', { error: e.message, method: 2 });
      }
    }
    ...
    try {
      ...
      for (let i = 0; i < editButtons.length; i++) {
        ...
        try {
          ...
          if (isVisible) {
            ...
            // Nota: o índice na lista pode começar em 0, mas os data-testid podem começar em 0 também
            try {
      ...
      logger.warn('Método 3 falhou', { error: e.message, method: 3 });
    }
    ...
    logger.error('Erro ao listar usuários', error, { function: 'listAllUsers' });
    ...
    try {
      ...
      logger.error('Erro no método alternativo', e2, { function: 'listAllUsers', method: 'alternativo' });
    }
  }
