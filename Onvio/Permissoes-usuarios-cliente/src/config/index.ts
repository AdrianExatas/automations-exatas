...
export const config = {
  ...
  timeouts: {
    short: parseInt(process.env.TIMEOUT_SHORT || '1000', 10),
    medium: parseInt(process.env.TIMEOUT_MEDIUM || '2000', 10),
    long: parseInt(process.env.TIMEOUT_LONG || '10000', 10),
    mfa: parseInt(process.env.TIMEOUT_MFA || '120000', 10)
  },

  // Configuração de retry
  retry: {
    maxRetries: parseInt(process.env.MAX_RETRIES || '3', 10),
    initialDelay: parseInt(process.env.RETRY_DELAY || '500', 10),
  ...
  cache: {
    ...
    ttl: parseInt(process.env.CACHE_TTL || '3600000', 10), // 1 hora em ms
...
try {
  ...
  console.error('Erro ao validar configuração:', (error as Error).message);
  ...
  Object.keys(updates).forEach(key => {
    if (config.hasOwnProperty(key)) {
      ...
      if (typeof configValue === 'object' && !Array.isArray(configValue)) {
        Object.assign(configValue, updates[key]);
      } else {
