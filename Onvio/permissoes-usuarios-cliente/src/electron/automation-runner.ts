...
import { extractUserData, extractDepartments, extractCompanies, extractPermissions, saveToExcel } from '../utils/extractUserData';
import { listAllUsers } from '../utils/listUsers';
import { logger } from '../utils/logger';
import { getModuleConfig } from '../config';
import { startMetrics, endMetrics, exportMetrics, generateMetricsReport } from '../utils/metrics';
import { generateFullReport } from '../utils/reportGenerator';
import { notifyAutomationComplete, notifyProgress, notifyCriticalError } from '../utils/notifications';
...
const testResultsDir = (automationConfig.directories as any)?.testResults || path.join(__dirname, '../../test-results');
if (!fs.existsSync(testResultsDir)) {
  fs.mkdirSync(testResultsDir, { recursive: true });
}

const logsDir = (automationConfig.directories as any)?.logs || path.join(__dirname, '../../logs');
 ...
 */
interface AutomationConfigType {
  LOGIN_URL: string;
...
const CONFIG: AutomationConfigType = {
...
async function runAutomation(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      logger.warn('Erro ao gerar relatórios', { error: (error as Error).message });
    }
 ...
 */
async function performLogin(page: Page, email: string, password: string, mfaMethod: string, mfaCode?: string): Promise<void> {
 ...
 */
async function handleMFA(page: Page, mfaMethod: string, mfaCode?: string): Promise<void> {
  try {
    await Promise.race([
    ...
    if (mfaPage) {
      ...
      if (mfaCode) {
        try {
          ...
          logger.warn('Não foi possível preencher código MFA automaticamente', { step: 'mfa', error: (e as Error).message });
        }
      } else {
 ...
 */
async function navigateToClientPortal(page: Page): Promise<Page> {
 ...
 */
async function navigateToClientUsers(page: Page): Promise<void> {
 ...
 */
async function selectClient(page: Page, clientId: string, maxRetries: number = 3): Promise<boolean> {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      logger.warn(`Tentativa ${attempt} falhou: ${(error as Error).message}`, { attempt, maxRetries, clientId, error: (error as Error).message });
 ...
 */
async function configurePagination(page: Page): Promise<void> {
  try {
    ...
    logger.warn('Não foi possível configurar paginação, continuando...', { error: (e as Error).message });
  }
}
 ...
 */
async function returnToUserList(page: Page): Promise<boolean> {
  try {
 ...
 */
async function processUser(page: Page, users: UserListItem[], index: number, clientId: string, allUsersData: UserData[]): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        logger.warn('Erro ao salvar planilha em tempo real', { error: (error as Error).message });
      }
    ...
    if ((error as Error).message && ((error as Error).message.includes('authentication') || (error as Error).message.includes('login'))) {
 ...
 */
async function navigateThroughTabs(page: Page, userData: UserData): Promise<void> {
  // Aba Departamentos
 ...
 */
async function saveFinalData(allUsersData: UserData[]): Promise<void> {
