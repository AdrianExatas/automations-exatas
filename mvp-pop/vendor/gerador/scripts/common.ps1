...
function ConvertTo-NormalizedContent {
  ...
  foreach ($entry in @(
    [PSCustomObject]@{ Label = 'Código POP'; Value = $codes.codigo_pop },
    [PSCustomObject]@{ Label = 'Código IT'; Value = $codes.codigo_it },
    [PSCustomObject]@{ Label = 'Código FORM'; Value = $codes.codigo_form },
    [PSCustomObject]@{ Label = 'Código MP'; Value = $codes.codigo_mp },
    ...
    if ($entry.Value -match '(?i)(XXX|XX/XX|Ponto para validação|#Ponto)') {
      $text = "Ponto para validação: confirmar $($entry.Label.ToLowerInvariant())."
      if (-not $pending.Contains($text)) { $pending.Add($text) }
    }
  }
  foreach ($risk in $risks) {
    if ($risk.sugerido) {
      $text = "Ponto para validação: revisar o risco sugerido $($risk.id) e preencher probabilidade e gravidade."
      if (-not $pending.Contains($text)) { $pending.Add($text) }
    }
  }
  ...
  $transcriptionRaw = Get-PropertyValue $Raw 'transcricao' ([PSCustomObject]@{ status = 'nao_executada'; idioma = 'pt' })
  $transcriptionStatusRaw = [string](Get-PropertyValue $transcriptionRaw 'status' 'nao_executada')
  ...
  $transcriptionStatus = switch ($transcriptionStatusRaw.Trim().ToLowerInvariant()) {
    { $_ -in @('success', 'concluida', 'concluído', 'concluido', 'fornecida') } { 'concluida'; break }
  ...
  return [PSCustomObject][ordered]@{
    ...
    pontos_validacao = @($pending.ToArray())
...
function Repair-Utf8Mojibake {
  ...
  try {
    ...
    # mantém o texto original se a correção falhar
  }
  return $Text
}
