  ...
  async complete(systemPrompt: string, userPrompt: string): Promise<string> {
    ...
    try {
      ...
      if (err instanceof Error && err.name === "AbortError") {
        throw new Error(`Timeout Ollama (${OLLAMA_TIMEOUT_MS / 1000}s)`);
      }
      throw err;
    } finally {
...
export async function checkOllamaConnection(): Promise<{
  ...
  try {
    const response = await fetch(`${OLLAMA_BASE_URL.replace(/\/$/, "")}/api/tags`);
    if (!response.ok) {
      return { ok: false, models: [], error: `Ollama retornou ${response.status}` };
    }
    ...
    if (!hasModel) {
      return {
        ok: false,
        models,
        error: `Modelo '${OLLAMA_MODEL}' não encontrado. Execute: ollama pull ${OLLAMA_MODEL}`,
      };
    }
    ...
    return {
      ok: false,
      models: [],
      error: err instanceof Error ? err.message : "Falha ao conectar Ollama",
    };
  }
}
