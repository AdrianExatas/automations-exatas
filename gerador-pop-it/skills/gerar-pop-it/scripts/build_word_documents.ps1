...
function Set-CellFill($Cell, [int]$Color) {
  # Word ColorFormat usa BGR inteiro; 16777215 = branco, 13553360 = cinza D0CECE.
  $Cell.Shading.BackgroundPatternColor = $Color
}

function Set-RowFill($Row, [int]$Color) {
  for ($col = 1; $col -le $Row.Cells.Count; $col++) {
    Set-CellFill $Row.Cells.Item($col) $Color
  }
}
...
function Build-PopDocument($Word, $Content, $Metadata, [string]$TemplatePath, [string]$OutputPath, [string]$LogoFile) {
  ...
  $steps = ConvertTo-ObjectArray (Get-PropertyValue $pop "etapas")
  if ($steps.Count -eq 0) {
  ...
  try {
    ...
    # Header cinza (D0CECE); linhas de preenchimento brancas.
    $headerGray = 13553360
    $fillWhite = 16777215
    Set-RowFill $procedureTable.Rows.Item(1) $headerGray
...
function Build-ItBody($Word, $Doc, $Content) {
  ...
  $sections = ConvertTo-ObjectArray (Get-FirstPropertyValue $it @("secoes", "etapas"))
