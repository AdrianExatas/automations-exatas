...
function Validate-WordDocuments {
  ...
  try {
    ...
    if ($null -ne $word) {
      try { $word.Quit() } catch { Write-Warning ("Nao foi possivel encerrar o Word: " + $_.Exception.Message) }
...
function Validate-ExcelDocuments {
  ...
  try {
    ...
    if ($null -ne $excel) {
      try { $excel.Quit() } catch { Write-Warning ("Nao foi possivel encerrar o Excel: " + $_.Exception.Message) }
