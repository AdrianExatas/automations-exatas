...
async function launchCdpBrowserSession(): Promise<BrowserSession> {
  ...
  const temporaryProfileDir = await mkdtemp(path.join(os.tmpdir(), "fap-consulta-chrome-"));
...
async function removeTemporaryProfile(profileDir: string): Promise<void> {
  ...
  if (path.dirname(resolved) !== expectedParent || !path.basename(resolved).startsWith("fap-consulta-chrome-")) {
