...
import { formatDateBr, parseIsoDate } from "../utils/dates.js";
...
import { aplicarResultadoSolicitacao } from "./resultado.js";
...
export async function executarCapturaContinuaHttp(options: {
  ...
  console.log(`Modo: transporte ${options.transport ?? "auto"}`);
  console.log("=".repeat(70));
  ...
  let client: SefazPortalClient | undefined;
  try {
    ...
    console.log("\n[ETAPA 1/3] Autenticando cliente HTTP com certificado digital...");
    client = new SefazPortalClient(options.transport ?? "auto", options.headless);
    await client.login(SEFAZ_CERT_PFX_PATH, SEFAZ_CERT_PFX_PASSWORD);
    console.log(`${SIMBOLOS.login} Login por certificado realizado`);
    ...
    for (const [idx, empresa] of empresas.entries()) {
      ...
      for (const [tipoArquivo, pesquisarPor] of combinacoes) {
        ...
        try {
          ...
          if (!aplicarResultadoSolicitacao({
            resultado,
            historico,
            chaveHistorico,
            dataProcessada: dataFinalIntervalo,
            tipoArquivo,
            pesquisarPor,
          })) {
            console.log(`[ERRO] ${resultado.mensagem.slice(0, 80)}`);
            totalErros += 1;
            continue;
          }
          console.log("[OK]");
