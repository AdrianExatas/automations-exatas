...
export class SiegCertificadoClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      const rawMsg =
        parsed.message ?? parsed.mensagem ?? parsed.Mensagem ?? text;
      if (Array.isArray(parsed.errors) && parsed.errors.length) {
        mensagem = parsed.errors.join("; ");
      } else if (Array.isArray(rawMsg)) {
        mensagem = rawMsg.length ? rawMsg.join("; ") : (text || `HTTP ${response.status}`);
      } else {
        mensagem = typeof rawMsg === "string" ? rawMsg : String(rawMsg ?? text);
      }
    ...
    // Respeitar indicador de sucesso do corpo (API pode retornar 200 com success: false)
    const bodyOk =
      typeof parsed?.success === "boolean"
        ? parsed.success
        : typeof (parsed as { ok?: boolean })?.ok === "boolean"
          ? (parsed as { ok: boolean }).ok
          : undefined;
    const ok =
      bodyOk !== undefined ? bodyOk && response.ok : response.ok;
   ...
   * Nome é obrigatório na API SIEG ("nome certificado não pode ser vazio!").
   */
  async cadastrarAtualizarCertificado(
    ...
    const payload: CertificadoRequest = {
      ...
      TipoCertificado: "Pfx",
      // Módulo gestão fiscal/hub: consultas NF-e, CT-e, NFS-e Portal Nacional, consulta noturna (igual ao Atualizar)
      // ConsultaNfce só disponível para RS; ConsultaSat só para SP (API SIEG)
      ConsultaNfe: true,
      ConsultaCte: true,
      ConsultaNfse: true,
      ConsultaNfce: ufNorm === "RS",
      ConsultaNoturna: true,
      TipoConsultaNfse: "Nacional",
      ConsultaSat: ufNorm === "SP",
    ...
    if (nomeTrim) {
      ...
      (payload as Record<string, unknown>).NomeCertificado = nomeTrim;
    }
    if (cnpj) payload.CnpjCpf = cnpj.replace(/[./\-]/g, "");
    if (uf) payload.UfCertificado = ufNorm;
